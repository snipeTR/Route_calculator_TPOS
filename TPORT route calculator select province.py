...
# Kullanıcıdan ziyaret edilecek şehirleri isteme ve doğrulama
def get_visiting_cities(cities, start_city):
    visiting_cities = [start_city]
    while True:
        visit_city = input("Ziyaret etmek istediğiniz bir ili girin (örn: DİYARBAKIR): ").strip().upper()
        if visit_city not in cities:
            print("Yanlış şehir ismi. Lütfen tekrar deneyin.")
        elif visit_city in visiting_cities:
            print("Bu şehri zaten ziyaret edeceksiniz. Lütfen başka bir şehir girin.")
        else:
            visiting_cities.append(visit_city)
            more = input("Başka bir il eklemek ister misiniz? (E/H): ").strip().upper()
            if more == 'H':
                break
...
def main():
    ...
    visiting_cities = get_visiting_cities(cities, start_city)
    visiting_indices = [cities.index(city) for city in visiting_cities]
    ...
    Z = np.copy(M)
    rota_mx = np.zeros(len(visiting_indices), dtype=int)
    t = 0
    ss = visiting_indices[0]

    visited = set()
    while t < len(visiting_indices):
        bx = np.argmin(Z[ss])
        while bx in visited or bx not in visiting_indices:
            Z[ss, bx] = np.inf
            bx = np.argmin(Z[ss])
        rota_mx[t] = bx
        visited.add(bx)
